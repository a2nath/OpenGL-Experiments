...
Camera::Camera()
	: m_Viewport( 0, 0, WINDOW_WIDTH, WINDOW_HEIGHT)
	, m_Position(g_InitialCameraPosition)
	, m_Rotation(g_InitialCameraRotation)
...
void Camera::SetPosition(const glm::vec3& pos, GLboolean set_initial)
{
	m_Position = pos;
	m_ViewDirty = true;
	if (set_initial) g_InitialCameraPosition = m_Position;
}
